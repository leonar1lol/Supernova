/* admin-users.js
   Fetch users from /admin/api/users and render table. Handles delete and toggleAdmin via POST.
*/
(function(){
    'use strict';
        ...
        tr.innerHTML = `
            ...
            <td class="text-center" data-label="Acciones">
                <div class="action-forms">
                    ...
                    <button class="action-btn btn-toggle small-btn" data-id="${u.id}" data-action="toggle" title="Alternar admin">
    ...
    function load(){
        ...
        fetch(base + '/admin/api/users', {cache:'no-store'}).then(function(r){
            ...
            tbody.innerHTML = '';
            if (!json.length){
                // mostrar fila vacía para indicar que no hay usuarios
                var tr = document.createElement('tr');
                tr.innerHTML = '<td colspan="5" style="text-align:center;color:#666;padding:20px">(No hay usuarios)</td>';
    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        if (btnNew){
            ...
            function closeCreate(){ cModal.style.display='none'; cuCreate.disabled=false; cuCreate.textContent='Crear'; }
                btnNew.addEventListener('click', function(e){ e.preventDefault(); cuNombre.value=''; cuEmail.value=''; cuRol.value='cliente'; cuPassword.value=''; cModal.style.display='flex'; });
            ...
            cuCreate.addEventListener('click', function(e){ e.preventDefault(); cuCreate.disabled=true; cuCreate.textContent='Creando...';
                var p = new URLSearchParams(); p.append('action','create'); p.append('nombre', cuNombre.value || ''); p.append('email', cuEmail.value || ''); p.append('rol', cuRol.value || 'cliente'); p.append('password', cuPassword.value || '');
        ...
        document.body.addEventListener('click', function(e){
            ...
            if (action === 'delete'){
                // use global confirm modal instead of native confirm
                ...
                postAction('toggleAdmin', id, function(res){ if(res && res.ok){ load(); } else { globalShowToast('Error al cambiar rol','error'); } });
            } else if (action === 'edit'){
                // open edit modal and prefill values from the row
                ...
                euRol.value = tr.children[3].textContent.trim();
                euPassword.value = '';
